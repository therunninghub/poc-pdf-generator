...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String inputFile = Objects.requireNonNull(classloader.getResource("templates/template_e-certificate_RMS.pdf")).getFile();
        File templateFile = new File(inputFile);

        File currDir = new File("generated");
        String path = currDir.getAbsolutePath();
        final Path outputPath = Files.createDirectories(Paths.get(path));
    ...
    private static void generatePDFile(File templateFile, List<TextOption> dataFieldData, String fileNamePrefix, Path outputPath) throws IOException {
        ...
        String fontDir = "fonts/roboto.ttf";
        ClassLoader classloader = Thread.currentThread().getContextClassLoader();
        InputStream fontStream = classloader.getResourceAsStream(fontDir);
        ...
        // Remove fields after adding text
        acroForm.flatten(new ArrayList<>(pdFieldToDataField.keySet()), false);
    ...
    private static List<FileData> getDataSet() {
        ...
        TextOption user1FullName = new TextOption("fullName", "Nguyễn Văn A", fullNameColor, "center");
        TextOption user1Distance = new TextOption("distance", "42km", distanceColor, "center");
        TextOption user2FullName = new TextOption("fullName", "Nguyễn Văn A", fullNameColor, "left");
        TextOption user2Distance = new TextOption("distance", "100km", distanceColor, "center");
        TextOption user3FullName = new TextOption("fullName", "Nguyễn Văn A", fullNameColor, "right");
